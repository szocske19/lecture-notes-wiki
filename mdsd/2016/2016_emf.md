...
1. Let's change two thing on the .genmodel file. 1) Click the root object on the .genmodel editor and find Model | Model Directory. Let's change the src folder to emf-gen (this will change the generation destination folder). 2) Click on the Package element and find All | Base Package. Change it to ``hu.bme.mit.mdsd``. This will correct the generated package names according to the project name. Regenerate the Model code, delete the previously generated code and correct the MAINFEST file as it tries to export the deleted packages.
...
1. Create a main method which the runs the previous code. Try it out by right clicking to the class and choosing **Run as | Java Application**.
...
1. Probably it won't run. Ecore models needs initialization, which can be done with following code. Note that this is not needed if the you run the code as an Eclipse application or as a JUnit Plug-in test.
...
1. The model can be retrieved from a ``Resource`` by the ``getContents()`` method content:
